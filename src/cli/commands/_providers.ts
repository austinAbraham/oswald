...
export function selectProviders(sel: ProviderSelection): ProviderSelectionResult {
  ...
  if (sel.repo) {
    const requestedRepo = sel.repoSettings?.provider === "git" ? "git" : "mock";
    const repoProvider = selectRepoProvider(sel.cwd, sel.repoSettings);
    providers.repo = repoProvider;
    const resolvedRepo = repoProvider.name === "git" ? "git" : "mock";
    const repoFallback = requestedRepo === "git" && resolvedRepo === "mock";
    if (repoFallback) {
      sel.audit?.record("provider_fallback", {
        provider: "repo",
        requested: "git",
        used: "mock",
        reason: "git CLI not found on PATH",
      });
    }
    resolution.push({
      slot: "repo",
      ...
      ...(repoFallback
        ? {
            reason: "git CLI not found on PATH",
            remediation:
              "install git to enable real branch/commit/PR operations, or set repo.provider: mock",
          }
        : {}),
    });
  }
