...
Operator commands (`doctor`, `ship`, `compact`, `brief`, `next`, `init`) use
`0`/`1` (`doctor` returns `1` on any fail-status check; `ship`/`compact`/`brief`
return `1` on a precondition failure).
